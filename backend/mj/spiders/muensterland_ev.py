...
class MuensterlandEvSpider(EventSpider):
    ...
    def start_requests(self):
        ...
        if not (os.getenv('DATENPORTAL_USER') and  os.getenv("DATENPORTAL_PASSWORD")):
            self.logger.error(
                "Please set the DATENPORTAL_USER and DATENPORTAL_PASSWORD environment variables")
        ...
        today = date.today()
        event_api_url = URL + '?filter[starts_after]=' + today.strftime('%Y-%m-%d') \
    ...
    def parse(self, response):
        for event in response.json()['data']:
            ...
            yield {
                ...
                'location': address,
                'performer': '',
                'mode': event['types'][0]['name'] if ('types' in event) and len(event['types'])>0 else '',
                'organizer': event['event_organizers'][0]['name'] if ('event_organizers' in event) and len(event['event_organizers'])>0 else ''
            }
